...
# Search specifically for bench press
url = "https://api.github.com/search/repositories?q=bench+press+counter"
req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
try:
    res = urllib.request.urlopen(req)
    repos = [r['full_name'] for r in json.loads(res.read()).get('items', [])[:5]]
    print("Bench press repos found:", repos)
    for repo in repos:
...
# Also try searching code directly
url = "https://api.github.com/search/code?q=bench+press+extension:mp4"
req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
try:
    res = urllib.request.urlopen(req)
    items = json.loads(res.read()).get('items', [])
    print("Code search items found:", len(items))
    for item in items[:5]:
        repo_name = item['repository']['full_name']
        path = item['path']
        print(f"  https://raw.githubusercontent.com/{repo_name}/master/{path}")
except Exception as e:
    print(e)
